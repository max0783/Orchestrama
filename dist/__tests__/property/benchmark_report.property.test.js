...
// Helpers
// ---------------------------------------------------------------------------
function createMockOllamaClient(generateFn) {
    return {
...
describe("Property 22: Benchmark report completeness", () => {
    it("report contains an entry for every model in the input list", async () => {
        await fc.assert(fc.asyncProperty(
        // Arbitrary non-empty list of model names (1–10 models)
        fc.array(fc.string({ minLength: 1, maxLength: 30 }).filter((s) => s.trim().length > 0), { minLength: 1, maxLength: 10 }), async (modelNames) => {
    ...
    it("each successful model entry includes latency, throughput, and responseLength fields", async () => {
        await fc.assert(fc.asyncProperty(
        // Arbitrary non-empty list of model names (1–5 models)
        fc.array(fc.string({ minLength: 1, maxLength: 20 }).filter((s) => /^[a-zA-Z0-9_-]+$/.test(s)), { minLength: 1, maxLength: 5 }), async (modelNames) => {
    ...
    it("failed model entries record status ERROR and continue with remaining models", async () => {
        await fc.assert(fc.asyncProperty(
        // At least 2 models so we can have one fail and one succeed
        fc.array(fc.string({ minLength: 1, maxLength: 20 }).filter((s) => /^[a-zA-Z0-9_-]+$/.test(s)), { minLength: 2, maxLength: 6 }), 
        // Index of the model that will fail (0-based)
        fc.nat({ max: 1 }), async (modelNames, failIndex) => {
    ...
    it("with iterations > 1, report includes mean metrics for each model", async () => {
        await fc.assert(fc.asyncProperty(
        // 1–3 models
        fc.array(fc.string({ minLength: 1, maxLength: 15 }).filter((s) => /^[a-zA-Z0-9_-]+$/.test(s)), { minLength: 1, maxLength: 3 }), 
        // 2–5 iterations
        fc.integer({ min: 2, max: 5 }), async (modelNames, iterations) => {
            const ollamaClient = createMockOllamaClient(async (_req) => ({
                ...FIXED_RESPONSE,
                model: _req.model,
            }));
...
//# sourceMappingURL=benchmark_report.property.test.js.map