...
describe("Property 17: .bridgeignore pattern exclusion", () => {
    it("files matching .bridgeignore patterns are excluded from readContextFiles output", async () => {
        ...
        fc.array(fc.stringMatching(/^[a-z][a-z0-9]{0,10}\.(txt|log|ts|js)$/).filter((s) => s.length > 0), { minLength: 2, maxLength: 8 }), async (filenames) => {
            // Deduplicate filenames
            const uniqueFilenames = [...new Set(filenames)];
            if (uniqueFilenames.length < 2)
                return;
    ...
    it("loadIgnoreRules includes default patterns regardless of .bridgeignore", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ maxLength: 100 }), async (extraContent) => {
...
//# sourceMappingURL=bridgeignore.property.test.js.map