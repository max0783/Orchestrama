...
const nonEmptyString = fc.string({ minLength: 1, maxLength: 60 }).filter((s) => s.trim() !== "");
// ---------------------------------------------------------------------------
// Property 15 tests
// ---------------------------------------------------------------------------
describe("Property 15: Capability map first-match routing", () => {
    it("returns the model for a pattern that appears in the prompt", () => {
        ...
        fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim() !== "" && !s.includes("\0")), nonEmptyString, // model for that pattern
    ...
    it("returns defaultModel when no pattern matches the prompt", () => {
        fc.assert(fc.property(nonEmptyString, // defaultModel
    ...
    it("returns the FIRST matching pattern's model (insertion order)", () => {
        ...
        fc.tuple(fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_-]{0,14}$/), fc.stringMatching(/^[a-zA-Z][a-zA-Z0-9_-]{0,14}$/)).filter(([a, b]) => a !== b && !a.includes(b) && !b.includes(a)), nonEmptyString, // model for first pattern
    ...
    it("matching is case-insensitive: uppercase pattern matches lowercase prompt", () => {
        ...
        fc.stringMatching(/^[a-z]{1,20}$/), nonEmptyString, // model
        nonEmptyString, // defaultModel
    ...
    it("matching is case-insensitive: lowercase pattern matches uppercase prompt", () => {
        fc.assert(fc.property(fc.stringMatching(/^[a-z]{1,20}$/), nonEmptyString, nonEmptyString, (lowerPattern, model, defaultModel) => {
    ...
    it("getMap() returns a copy of the capability map", () => {
        fc.assert(fc.property(fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim() !== ""), nonEmptyString, { minKeys: 0, maxKeys: 5 }), nonEmptyString, (capMap, defaultModel) => {
            const router = new CapabilityRouter(capMap, defaultModel);
...
//# sourceMappingURL=capability_routing.property.test.js.map