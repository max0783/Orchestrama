...
describe("Property 8: Chunk size invariant", () => {
    it("every chunk has token estimate <= maxTokens", () => {
        ...
        fc.string({ minLength: 1, maxLength: 10000 }), (contextWindow, payload) => {
    ...
    it("chunks cover the full payload (no content lost)", () => {
        fc.assert(fc.property(fc.integer({ min: 10, max: 500 }), fc.string({ minLength: 1, maxLength: 5000 }), (contextWindow, payload) => {
            const maxTokens = Math.floor(contextWindow * 0.9);
            if (maxTokens <= 0)
                return true;
            const chunks = splitIntoChunks(payload, maxTokens);
...
//# sourceMappingURL=chunk_size.property.test.js.map