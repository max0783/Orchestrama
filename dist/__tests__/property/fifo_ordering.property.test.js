...
describe("Property 20: FIFO queue ordering", () => {
    it("dispatches tasks in the order they were enqueued (concurrency=1)", async () => {
        await fc.assert(fc.asyncProperty(
        // N tasks: between 2 and 10
        fc.integer({ min: 2, max: 10 }), async (n) => {
            ...
            const tasks = Array.from({ length: n }, (_, i) => () => new Promise((resolve) => {
            ...
            const promises = tasks.map((task) => queue.enqueue(task, 5000));
...
//# sourceMappingURL=fifo_ordering.property.test.js.map