...
// ---------------------------------------------------------------------------
async function assertInvalidParams(args) {
    ...
    expect(thrownError.code).toBe(ErrorCode.InvalidParams);
}
// ---------------------------------------------------------------------------
...
describe("Property 2: Invalid parameter rejection", () => {
    it("rejects when prompt is a number", async () => {
        await fc.assert(fc.asyncProperty(fc.double({ noNaN: true }), async (promptValue) => {
            await assertInvalidParams({ prompt: promptValue });
        }), { numRuns: 100 });
    }, 15_000);
    it("rejects when prompt is a boolean", async () => {
        await fc.assert(fc.asyncProperty(fc.boolean(), async (promptValue) => {
            await assertInvalidParams({ prompt: promptValue });
        }), { numRuns: 100 });
    }, 15_000);
    ...
    it("rejects when context_files is a plain string (not an array)", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 200 }), // valid prompt
    ...
    it("rejects when context_files is a number", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 200 }), fc.integer(), async (prompt, contextFiles) => {
            await assertInvalidParams({ prompt, context_files: contextFiles });
        }), { numRuns: 100 });
    }, 15_000);
    it("rejects when context_files is an array containing non-string elements", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 200 }), 
        // Array with at least one number element
        fc.array(fc.integer(), { minLength: 1, maxLength: 5 }), async (prompt, contextFiles) => {
            await assertInvalidParams({ prompt, context_files: contextFiles });
        }), { numRuns: 100 });
    }, 15_000);
    it("rejects when context_files is an array mixing strings and numbers", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 200 }), fc.string({ minLength: 1, maxLength: 50 }), fc.integer(), async (prompt, validPath, invalidItem) => {
    ...
    it("accepts valid prompt string with no context_files", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 200 }), async (prompt) => {
            const deps = createMockDeps();
            const handler = createQueryHandler(deps);
            // Should not throw
    ...
    it("accepts valid prompt string with valid context_files array of strings", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 200 }), fc.array(fc.string({ minLength: 1, maxLength: 50 }), { maxLength: 3 }), async (prompt, contextFiles) => {
            const deps = createMockDeps();
            const handler = createQueryHandler(deps);
            // Should not throw
...
//# sourceMappingURL=invalid_params.property.test.js.map