...
function makeSuccessResponse(model) {
    return {
        model,
        response: "ok",
        context: [],
...
describe("Property 14: Keep-alive field presence", () => {
    ...
    it("every generate() call includes keep_alive equal to the configured value", async () => {
        ...
        fc.oneof(fc.constant("5m"), fc.constant("10m"), fc.constant("30m"), fc.constant("1h"), fc.constant("0"), fc.constant("-1"), fc.stringMatching(/^\d+[smh]$/)), 
        // Arbitrary model names
        fc.string({ minLength: 1, maxLength: 50 }), 
        // Arbitrary prompts
        fc.string({ maxLength: 200 }), async (keepAlive, model, prompt) => {
    ...
    it("keep_alive in request body matches config even when request specifies a different keep_alive", async () => {
        await fc.assert(fc.asyncProperty(fc.oneof(fc.constant("5m"), fc.constant("10m"), fc.constant("1h"), fc.stringMatching(/^\d+[smh]$/)), fc.oneof(fc.constant("1m"), fc.constant("2h"), fc.stringMatching(/^\d+[smh]$/)), async (configuredKeepAlive, requestKeepAlive) => {
            const capturedBodies = [];
            const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
                capturedBodies.push(init?.body);
                return {
                    ok: true,
                    status: 200,
                    text: async () => JSON.stringify(makeSuccessResponse("llama3")),
...
//# sourceMappingURL=keepalive_presence.property.test.js.map