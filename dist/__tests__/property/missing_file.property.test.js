...
describe("Property 6: Missing file error marker", () => {
    ...
    it("remaining files are still processed when some are missing", () => {
        ...
        }), { minLength: 1, maxLength: 10 }), fc.array(fc.string({ minLength: 1, maxLength: 80 }), { minLength: 1, maxLength: 10 }), (existingPairs, missingPaths) => {
            const reader = new FileReader(["/allowed"]);
            const existingResults = existingPairs.map(({ path, content }) => ({
                path,
                content,
                error: undefined,
                tokenEstimate: Math.floor(content.length / 4),
            }));
            const missingResults = missingPaths.map((p) => ({
                path: p,
                content: null,
                error: "file not found",
                tokenEstimate: 0,
...
//# sourceMappingURL=missing_file.property.test.js.map