...
const nonEmptyString = fc.string({ minLength: 1, maxLength: 80 }).filter((s) => s.trim() !== "");
/** Arbitrary capability map with 1–5 entries. */
const capabilityMapArb = fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim() !== ""), nonEmptyString, { minKeys: 1, maxKeys: 5 });
// ---------------------------------------------------------------------------
// Property 10 tests
// ---------------------------------------------------------------------------
describe("Property 10: Model resolution precedence", () => {
    it("(1) explicit model always wins regardless of capability map or default", () => {
        fc.assert(fc.property(capabilityMapArb, nonEmptyString, // defaultModel
    ...
    it("(2) capability-map first match wins when no explicit model is given", () => {
        ...
        fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim() !== "" && !s.includes("\0")), nonEmptyString, // model associated with the pattern
    ...
    it("(4) falls back to 'llama3.1:8b' when defaultModel is empty and no match", () => {
        fc.assert(fc.property(fc.oneof(fc.constant(""), fc.constant("   ")), fc.string({ maxLength: 200 }), (emptyDefault, prompt) => {
            // Use a pattern that cannot appear in the prompt
            const capMap = { "\x01\x02\x03": "some-model" };
    ...
    it("explicit empty/whitespace-only model is NOT treated as explicit — falls through to map/default", () => {
        fc.assert(fc.property(nonEmptyString, // defaultModel
        fc.string({ maxLength: 200 }), // prompt
        fc.oneof(fc.constant(""), fc.constant("   ")), (defaultModel, prompt, emptyExplicit) => {
...
//# sourceMappingURL=model_resolution.property.test.js.map