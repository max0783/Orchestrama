...
describe("Property 19: OOM error classification and fallback", () => {
    it("retries with fallback model when primary model throws OOM error", async () => {
        ...
        fc.string({ minLength: 1, maxLength: 500 }), async (prompt, oomSubstring, fallbackModel, fallbackResponse) => {
            ...
            expect(result.content[0].text).toContain(`[FALLBACK: ${fallbackModel} used due to resource exhaustion on ${primaryModel}]`);
            // And the actual fallback response
            expect(result.content[0].text).toContain(fallbackResponse);
        }), { numRuns: 100 });
    }, 30_000);
    it("uses the first fallback model from BRIDGE_FALLBACK_MODELS", async () => {
        ...
        fc.string({ minLength: 1, maxLength: 30 }).filter((s) => s.trim() !== "" && s !== "llama3"), async (prompt, firstFallback, secondFallback) => {
            ...
            expect(result.content[0].text).toContain(`[FALLBACK: ${firstFallback} used due to resource exhaustion on ${primaryModel}]`);
            // The second fallback should NOT have been used
            expect(modelsUsed).not.toContain(secondFallback);
        }), { numRuns: 100 });
    }, 30_000);
    it("skips fallback model if it is the same as the primary model", async () => {
...
//# sourceMappingURL=oom_fallback.property.test.js.map