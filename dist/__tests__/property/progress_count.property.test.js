...
describe("Property 16: Progress notification count during Map-Reduce", () => {
    it("emits exactly N chunk_done notifications for a payload requiring N chunks", async () => {
        ...
        fc.string({ minLength: 600, maxLength: 4000 }), async (contextWindow, payload) => {
            const systemPrompt = "Summarize concisely.";
            const systemPromptTokens = estimateTokens(systemPrompt);
            // Determine how many chunks the Map phase will produce
            const maxChunkTokens = Math.floor(contextWindow * 0.9) - systemPromptTokens;
            ...
            await chunker.process(payload, systemPrompt, {
                contextWindow,
                systemPromptTokens,
                model: "llama3",
            }, (event) => {
            ...
            const chunkDoneNotifications = notifications.filter((n) => n.method === "notifications/progress" &&
    ...
    it("emits zero chunk_done notifications when disabled (BRIDGE_DISABLE_PROGRESS=true)", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 40, max: 200 }), fc.string({ minLength: 600, maxLength: 3000 }), async (contextWindow, payload) => {
...
//# sourceMappingURL=progress_count.property.test.js.map