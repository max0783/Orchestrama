...
describe("Property 1: Response pass-through integrity", () => {
    it("returns the exact response string from Ollama in the MCP content field", async () => {
        await fc.assert(fc.asyncProperty(
        // Non-empty prompt
        fc.string({ minLength: 1, maxLength: 500 }), 
        // Arbitrary response string (including empty, unicode, multiline)
        fc.string({ maxLength: 2000 }), async (prompt, ollamaResponse) => {
            const mockGenerate = vi.fn(async (_req) => ({
                model: "llama3",
    ...
    it("response is not truncated even for long response strings", async () => {
        await fc.assert(fc.asyncProperty(
        // Non-empty prompt
        fc.string({ minLength: 1, maxLength: 200 }), 
        // Long response string
        fc.string({ minLength: 500, maxLength: 5000 }), async (prompt, ollamaResponse) => {
            const mockGenerate = vi.fn(async (_req) => ({
                model: "llama3",
                response: ollamaResponse,
                context: [],
...
//# sourceMappingURL=response_passthrough.property.test.js.map