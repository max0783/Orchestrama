...
describe("Property 5: Security rejection for out-of-bounds paths", () => {
    it("formatForPayload includes SECURITY ERROR marker for out-of-bounds paths", () => {
        fc.assert(fc.property(fc.array(fc.string({ minLength: 1, maxLength: 100 }), { minLength: 1, maxLength: 20 }), (paths) => {
    ...
    it("security error entries contain no file content", () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 100 }), 
        // Use a content string that is unlikely to appear in the error message
        fc.string({ minLength: 10, maxLength: 200 }).filter((s) => !s.includes("[SECURITY ERROR") && !s.includes("### File:")), (outPath, someContent) => {
            const reader = new FileReader(["/allowed/dir"]);
            const results = [
                {
                    path: outPath,
    ...
    it("isPathAllowed returns false for paths outside allowed dirs", () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }).filter((s) => !s.includes("\0") && !s.includes("/")), fc.string({ minLength: 1, maxLength: 50 }).filter((s) => !s.includes("\0") && !s.includes("/")), (allowedSuffix, outsideSuffix) => {
...
//# sourceMappingURL=security_rejection.property.test.js.map