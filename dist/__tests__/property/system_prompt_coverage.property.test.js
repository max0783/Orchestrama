...
// Helpers
// ---------------------------------------------------------------------------
const allTaskTypes = fc.constantFrom("code_review", "log_analysis", "summarization", "generic");
...
describe("Property 11: System prompt task-type coverage", () => {
    it("code_review prompt contains 'findings'", () => {
        fc.assert(fc.property(allTaskTypes, (taskType) => {
    ...
    it("generic prompt contains instructions to omit preamble/filler", () => {
        fc.assert(fc.property(allTaskTypes, (taskType) => {
            ...
            const hasPreambleOrFiller = lower.includes("preamble") ||
...
describe("Property 12: System prompt override precedence", () => {
    it("override param takes precedence over env var and built-in default", () => {
        fc.assert(fc.property(allTaskTypes, nonBlankString, // override param
    ...
    it("env var takes precedence over built-in default when no override param", () => {
        fc.assert(fc.property(allTaskTypes, nonBlankString, // env system prompt
        (taskType, envPrompt) => {
            const injector = new SystemPromptInjector(envPrompt);
    ...
    it("blank env var (whitespace only) does not override built-in default", () => {
        fc.assert(fc.property(allTaskTypes, fc.string({ minLength: 1, maxLength: 20 }).map((s) => s.replace(/\S/g, " ")), // whitespace-only env
        (taskType, blankEnv) => {
            if (blankEnv.trim() !== "")
                return;
    ...
    it("higher-priority value completely replaces lower-priority one (no merging)", () => {
        fc.assert(fc.property(allTaskTypes, nonBlankString, // override
        nonBlankString, // env
        (taskType, override, envPrompt) => {
...
//# sourceMappingURL=system_prompt_coverage.property.test.js.map