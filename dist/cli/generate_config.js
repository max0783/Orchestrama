...
// ---------------------------------------------------------------------------
function printForClient(client) {
...
// Supported clients
// ---------------------------------------------------------------------------
const SUPPORTED_CLIENTS = ["claude-desktop", "claude-code", "kiro", "cursor", "codex", "generic"];
function isSupportedClient(value) {
    return SUPPORTED_CLIENTS.includes(value);
}
// ---------------------------------------------------------------------------
// Main logic
// ---------------------------------------------------------------------------
...
if (clientArg !== undefined) {
    ...
    if (!isSupportedClient(clientArg)) {
        process.stderr.write(`Error: Unsupported client "${clientArg}". Supported clients: ${SUPPORTED_CLIENTS.join(", ")}\n`);
        process.exit(1);
    }
