...
async function checkPrerequisites(ollamaUrl) {
...
async function pickModel(ollamaUrl) {
...
async function detectClients(clients) {
...
function buildServerEntry(model, ollamaUrl) {
    return {
        command: "node",
        args: [SERVER_PATH],
        env: {
...
async function writeJsonConfig(client, model, ollamaUrl) {
    ...
    const servers = existing[client.mergeKey] ?? {};
...
async function writeTomlConfig(client, model, ollamaUrl) {
    await ensureDir(path.dirname(client.configPath));
...
async function writeConfigs(detected, model, ollamaUrl) {
    ...
    if (detected.length === 0) {
        ...
        await fs.writeFile(out, JSON.stringify({ mcpServers: { "ollama-mcp-bridge": buildServerEntry(model, ollamaUrl) } }, null, 2), "utf-8");
...
async function runTests(model, ollamaUrl) {
...
function printSystemPrompt(model) {
