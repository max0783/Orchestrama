 ...
 * (NaN or non-positive).
 */
function parsePositiveInt(name, raw, defaultValue) {
    if (raw === undefined || raw === "") {
        return defaultValue;
    }
    const parsed = Number(raw);
    if (Number.isNaN(parsed) || parsed <= 0) {
        process.stderr.write(`[ollama-mcp-bridge] ERROR: ${name} must be a positive integer, got: "${raw}"\n`);
        process.exit(1);
    }
    return parsed;
}
...
function parseCapabilityMap(raw) {
    ...
    try {
        ...
        if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
            process.stderr.write(`[ollama-mcp-bridge] ERROR: BRIDGE_CAPABILITY_MAP must be a JSON object, got: ${raw}\n`);
            return {};
        }
        return parsed;
    }
    catch {
        process.stderr.write(`[ollama-mcp-bridge] ERROR: Failed to parse BRIDGE_CAPABILITY_MAP as JSON: ${raw}\n`);
        return {};
    }
}
/**
...
export function loadConfig() {
    ...
    const contextWindow = parsePositiveInt("OLLAMA_CONTEXT_WINDOW", env["OLLAMA_CONTEXT_WINDOW"], 4096);
    const numParallel = parsePositiveInt("OLLAMA_NUM_PARALLEL", env["OLLAMA_NUM_PARALLEL"], 1);
    const queueMaxSize = parsePositiveInt("BRIDGE_QUEUE_MAX_SIZE", env["BRIDGE_QUEUE_MAX_SIZE"], 10);
    const requestTimeoutMs = parsePositiveInt("BRIDGE_REQUEST_TIMEOUT_MS", env["BRIDGE_REQUEST_TIMEOUT_MS"], 300000);
    const allowedDirsRaw = env["BRIDGE_ALLOWED_DIRS"];
    const allowedDirs = allowedDirsRaw && allowedDirsRaw.trim() !== ""
    ...
    const fallbackModels = fallbackModelsRaw && fallbackModelsRaw.trim() !== ""
    ...
    const logLevel = logLevelRaw === "debug" ? "debug" : "info";
