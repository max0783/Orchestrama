...
export class CapabilityRouter {
    capabilityMap;
    defaultModel;
    constructor(capabilityMap, defaultModel) {
     ...
     */
    resolveModel(prompt, explicitModel) {
        // 1. Explicit model wins
        if (explicitModel && explicitModel.trim() !== "") {
            process.stderr.write(`[ollama-mcp-bridge] Model: ${explicitModel} (explicit)\n`);
        ...
        for (const [pattern, model] of Object.entries(this.capabilityMap)) {
            if (lowerPrompt.includes(pattern.toLowerCase())) {
                process.stderr.write(`[ollama-mcp-bridge] Capability map match: pattern="${pattern}" → model="${model}"\n`);
                return model;
            }
        }
        // 3. Default model (or ultimate fallback "llama3.1:8b")
        const resolved = this.defaultModel && this.defaultModel.trim() !== ""
            ? this.defaultModel
            : "llama3.1:8b";
