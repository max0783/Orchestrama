...
// ---------------------------------------------------------------------------
async function checkOllamaConnectivity(ollamaClient, defaultModel) {
    ...
    try {
        ...
        return {
            ...
            resolution: "Ensure Ollama is running (e.g. `ollama serve`) and OLLAMA_BASE_URL is correct.",
        };
    }
}
async function checkDefaultModelExists(ollamaClient, defaultModel) {
    const start = Date.now();
    try {
...
async function checkEndToEndCall(ollamaClient, defaultModel) {
    const start = Date.now();
    try {
...
async function checkChunkingMechanism(chunker, defaultModel, contextWindow) {
    const start = Date.now();
    try {
        ...
        const result = await chunker.process(syntheticPayload, systemPrompt, opts, () => { } // no-op progress callback
        );
        const responseTimeMs = Date.now() - start;
...
function checkQueueStatus(requestQueue) {
...
function formatReport(report) {
    const lines = ["Configuration Test Report", "=".repeat(40)];
    for (const check of report.checks) {
        const icon = check.status === "PASS"
            ...
            : check.status === "SKIPPED"
                ? "⏭️"
                : "❌";
        const timeStr = check.responseTimeMs !== undefined ? ` [${check.responseTimeMs}ms]` : "";
    ...
    lines.push("\n" + "=".repeat(40));
    lines.push(report.allPassed
        ? "✅ All checks passed"
        : "❌ Some checks failed — see details above");
    return lines.join("\n");
}
export function createTestConfigHandler(deps) {
    ...
    return async (args) => {
        ...
        const allPassed = checks.every((c) => c.status === "PASS" || c.status === "SKIPPED");
        const report = { checks, allPassed };
