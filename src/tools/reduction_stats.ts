 ...
 */
export function createReductionStatsHandler(reductionLogger: IReductionLogger) {
  return async (_args: unknown) => {
    ...
    const text = [
      ...
      ...Object.entries(stats.byModel).map(
        ([m, s]) =>
          `  ${m}: ${s.invocations} invocations, avg ratio ${s.averageReductionRatio.toFixed(3)}`
      ),
      `\nBy task type:`,
      ...Object.entries(stats.byTaskType).map(
        ([t, s]) =>
          `  ${t}: ${s.invocations} invocations, avg ratio ${s.averageReductionRatio.toFixed(3)}`
      ),
    ].join("\n");
